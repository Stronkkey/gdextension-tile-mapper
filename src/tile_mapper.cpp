...
Ref<TileSetAtlasSource> TileMapper::get_atlas_source(const int32_t source_id) const {
...
Ref<Texture2D> TileMapper::get_texture_from_source_id(const int32_t source_id) const {
  if (tile_set.is_null() || !tile_set->has_source(source_id))
    return Ref<Texture2D>();
  Ref<TileSetAtlasSource> source = get_atlas_source(source_id);
  return source.is_valid() ? source->get_texture() : Ref<Texture2D>();
}

Rect2i TileMapper::get_texture_region_from_atlas_source(const int32_t source_id, const Vector2i &atlas_coords) const {
  Ref<TileSetAtlasSource> source = get_atlas_source(source_id);
  return source.is_valid() ? source->get_tile_texture_region(atlas_coords) : Rect2i();
}

Rect2i TileMapper::get_texture_region_from_cell_data(CellData *cell_data) const {
  return get_texture_region_from_atlas_source(cell_data->tile_info.source_id, Vector2i(cell_data->tile_info.x, cell_data->tile_info.y));
}


RID TileMapper::create_shape_for_cell_layer_polygon_index(CellData *cell_data, int32_t layer, int32_t polygon_index) const {
...
RID TileMapper::create_cell_body_for_layer(CellData *cell_data, int32_t layer) const {
  ...
  for (int32_t polygon_index = 0; polygon_index < cell_data->tile_data->get_collision_polygons_count(layer); polygon_index++) {
    RID shape = create_shape_for_cell_layer_polygon_index(cell_data, layer, polygon_index);
    if (shape == RID())
      continue;
  ...
  Size2i cell_size = get_texture_region_from_cell_data(cell_data).get_size();
...
TypedArray<RID> TileMapper::create_physics_bodies_for_cell(CellData *cell_data) const {
  TypedArray<RID> bodies = {};

  for (int32_t layer = 0; layer < tile_set->get_physics_layers_count(); layer++) {
    RID body = create_cell_body_for_layer(cell_data, layer);
    if (body != RID())
      bodies.append(body);
  }
...
void TileMapper::draw_quadrant(Quadrant *quadrant) {
  ...
  for (std::pair<int64_t, CellData*> iterator: quadrant->cells) {
    draw_quadrant_cell(iterator.second, quadrant);
  }
}

void TileMapper::draw_quadrant_cell(CellData *cell_data, Quadrant *quadrant) {
  Rect2i size_rect = get_texture_region_from_cell_data(cell_data);
...
Quadrant *TileMapper::create_new_quadrant() const {
...
bool TileMapper::should_draw_debug_shapes() const {
...
void TileMapper::cell_draw_debug_shape(CellData *cell_data, const Color &shape_color) {
  for (int i = 0; i < cell_data->physics_bodies_rid.size(); i++) {
    ...
    for (int shape_index = 0; i < shape_count; i++) {
      draw_cell_shape(cell_data, body, shape_index, shape_color);
    }
  }
}

void TileMapper::draw_cell_shape(CellData *cell_data, const RID &body, const int shape_index, const Color &shape_color) {
  Size2i cell_size = get_texture_region_from_cell_data(cell_data).get_size();
  ...
  RID draw_rid = get_draw_rid_from_cell_data(cell_data);
...
RID TileMapper::get_draw_rid_from_cell_data(CellData *cell_data) const {
  CellDrawState cell_draw_state = get_cell_draw_state(cell_data);
...
TileMapper::CellDrawState TileMapper::get_cell_draw_state(CellData *cell_data) const {
  if (cell_data != nullptr && cell_data->canvas_rid != RID())
    return CANVAS_ITEM;
  return cell_data->current_quadrant != nullptr ? QUADRANT : NONE;
}

bool TileMapper::destroy_quadrant(Quadrant *quadrant) {
  if (quadrant == nullptr)
    return false;
  ...
  for (std::pair<int64_t, CellData*> iterator: quadrant_cells) {
    iterator.second->current_quadrant = nullptr;
    destroy_cell(iterator.first);
  }
...
void TileMapper::remove_cell(CellData *cell_data, const bool remove_quadrant) {
  ...
  /*if (remove_quadrant && cell_quadrant) {
    ...
    if (cell_quadrant->cells.empty())
      destroy_quadrant(cell_quadrant);
    else
      draw_quadrant(cell_quadrant);
  }*/
...
Quadrant *TileMapper::get_quadrant_with_tile_info(TileInfo tile_info) {
  ...
  if (!cell_quadrants.empty()) {
    ...
    Quadrant *quadrant = last_quadrant->cells.size() >= quadrant_size ? create_new_quadrant() : last_quadrant;
    Ref<TileSetAtlasSource> source = get_atlas_source(tile_info.source_id);
  ...
  Quadrant *new_quadrant = create_new_quadrant();
  std::vector<Quadrant*> new_quadrant_vector = {new_quadrant};
  quadrants.insert({tile_info, new_quadrant_vector});
  return new_quadrant;
}

void TileMapper::create_new_cell(const Vector2 &coords, const int32_t source_id, const Vector2i &atlas_coords, const int alternative_tile_id) {
  ...
  cell_data->texture = get_texture_from_source_id(source_id);
  ...
  cell_data->physics_bodies_rid = create_physics_bodies_for_cell(cell_data);

  Quadrant *quadrant = get_quadrant_with_tile_info(tile_info);

  quadrant->cells.insert({cell_id, cell_data});
  tiles.insert({cell_id, cell_data});

  cell_data->current_quadrant = quadrant;
  draw_quadrant_cell(cell_data, quadrant);

  if (should_draw_debug_shapes())
    cell_draw_debug_shape(cell_data, shape_color);
}
...
int64_t TileMapper::add_cell(const Vector2 &coords, const int32_t source_id, const Vector2i &atlas_coords, const int alternative_tile_id) {
  ...
  int64_t cell_id = index;
  create_new_cell(coords, source_id, atlas_coords, alternative_tile_id);
...
void TileMapper::clear_cells() {
  ...
  for (auto iterator: local_quadrants) {
    ...
    for (Quadrant *quadrant: local_local_quadrants)
      destroy_quadrant(quadrant);
  }

  for (auto iterator: tiles) {
    remove_cell(iterator.second, false);
  }
